...
class Woocommerce_Doordash_Pickup_Location {
	...
	public function get_delivery_days() {
		...
		$tomorrow = date( 'Y-m-d', time() + DAY_IN_SECONDS + $gmt_offset );
		...
		while ( $i <= $number_of_days ) {
			...
			if ( ! empty( $day_hours ) ) {
				if ( date('Y-m-d', $current_day ) == $today ) {
					// Custom label for Today
					$days[$current_day] = date( $day_fmt, $current_day ).__( ' Today', 'local-delivery-by-doordash' );
				} else if ( date('Y-m-d', $current_day ) == $tomorrow ) {
					// Custom label for Tomorrow
					$days[$current_day] = date( $day_fmt, $current_day ).__( ' Tomorrow', 'local-delivery-by-doordash' );
				} else {
