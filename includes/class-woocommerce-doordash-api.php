...
class Woocommerce_Doordash_API {
	...
	public function accept_delivery_quote( &$delivery ) {
		...
		switch ( wp_remote_retrieve_response_code( $response ) ) {
			...
			case 409: 
			case 500: 
				WCDD()->log->error( sprintf( __( 'Error %d for /drive/v2/quotes/%s/accept' ), wp_remote_retrieve_response_code($response), $delivery->get_id() ) );
				$body = json_decode( wp_remote_retrieve_body( $response ) );
				WCDD()->log->error( $body->message );
		}
	...
	public function request( $request_path, $request_args ) {
		...
		if ( $response_code !== 200 ) {
			...
			switch ( $response_code && isset( $_POST['billing_first_name'] ) ) { //field doesn't exist in POST when coming from cart
				case 400: 
