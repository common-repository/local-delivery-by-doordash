...
class Woocommerce_Doordash_Delivery {
	...
	public function create_from_session() {
		...
		$data = array(
			...
			'pickup_phone_number'                => $this->format_phone( $location->get_phone_number() ),
	...
	public function format_phone( $phone ) {
		if ( ! empty( $phone ) ) {
			...
			// Drive API requires phone, use a dummy if it's not set yet
			switch( substr( get_locale(), -2 ) ) {
				case 'GB':
					$phone = '442000000000'; 
				...
				case 'AU':
					...
					break;
				default:
					$phone = '18004444444'; $phone = '61200000000'; 
			}
		}

		if ( strlen( $phone ) == 10 && substr( get_locale(), -2 ) == 'US' ) $phone = '1' . $phone;
